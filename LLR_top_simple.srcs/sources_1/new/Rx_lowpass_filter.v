// 40阶低通滤波器
module rx_lowpass_filter (
    input               clk,
    input               rst,
    input  signed [15:0]       data_in,        // (1,16,15) 有符号16位，15位小数
    input               din_vd,
    output  reg [15:0]  data_out,       // (1,16,15) 输出
    output  reg         dout_vd
);
...
// 移位寄存器 - 41阶滤波器
(* KEEP = "{TRUE|FALSE |SOFT}"*) reg [15:0] din_sreg [0:40];  // 存储(1,16,15)格式数据

// 折叠相加后的输入
(* KEEP = "{TRUE|FALSE |SOFT}"*) reg signed [16:0] prodt_din [0:20];  //(1,17,16)

// 乘法器输出
(* KEEP = "{TRUE|FALSE |SOFT}"*) reg signed [32:0] prodt_L [0:20];  // (1,33,31)

// 截位后的乘积
(* KEEP = "{TRUE|FALSE |SOFT}"*) wire signed [16:0] prodt_dout [0:20];   // (1,17,16)

// 加法树中间结果
(* KEEP = "{TRUE|FALSE |SOFT}"*) wire signed [16:0] add_s1 [0:10];  // 第一级加法
(* KEEP = "{TRUE|FALSE |SOFT}"*) wire signed [16:0] add_s2 [0:5];   // 第二级加法
(* KEEP = "{TRUE|FALSE |SOFT}"*) reg signed  [16:0] add_s3 [0:2];   // 第三级加法
(* KEEP = "{TRUE|FALSE |SOFT}"*) wire signed [16:0] add_s4 [0:1];   // 第四级加法
(* KEEP = "{TRUE|FALSE |SOFT}"*) wire signed [16:0] dout_L;          // 最终和
(* KEEP = "{TRUE|FALSE |SOFT}"*) wire signed [16:0] comp_dout_L;     // 补偿后的和
...
always @(posedge clk or posedge rst) begin
    if (rst) begin
        ...
        dout_vd <= din_vd_reg[2]; //|| din_vd_reg[42];
    end
end
...
always @(*) begin
    ...
    for (i = 0; i < 20; i = i + 1) begin
        prodt_din[i] = {din_sreg[i][15], din_sreg[i]} + 
                      {din_sreg[40-i][15], din_sreg[40-i]};
    end
    // 中间系数（第20个，索引20）
    prodt_din[20] = {din_sreg[20][15], din_sreg[20]};
end
...
generate
    for (genvar i = 0; i < 21; i = i + 1) begin : prod_cut
        assign  prodt_dout[i] = {prodt_L[i][32], prodt_L[i][30:15]};//截掉溢出的保护位和低位
    end
endgenerate
